...
```{r figure2-estimated-distribution-dynamic, fig.path='static-figures/', fig.show='hide', fig.height=5, cache=FALSE, eval=TRUE, dev='tiff'}
data(kde_smallpox) 
data(kde_ebola)
data(kde_mers)
...
```{r figure3-scaled-prob-symptoms, fig.path='static-figures/', fig.show='hide', fig.height=3, dev='tiff'}
...
```{r figure4-optimized-cost-analysis, fig.path='static-figures/', fig.show='hide', dev='tiff'}
durs <- seq(.5, 5, by=.1)
phis <- c(1/1000, 1/10000)
...
```{r supp-fig2-optimized-cost-analysis, fig.path='static-figures/', fig.show='hide'}
...
`r paste0("$", round(min_costs[which(min_costs[,"phi"]==1e-3), "min_cost"],0) - round(min_costs2[which(min_costs2[,"phi"]==1e-3), "min_cost"],0))` 
