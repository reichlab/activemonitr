...
$\Sexpr{my_round(mean(pstr_gamma_params_ebola$median),1)}$ 
days (95\% CI: 
$\Sexpr{my_round(quantile(pstr_gamma_params_ebola$median, .025),1)}$-$\Sexpr{my_round(quantile(pstr_gamma_params_ebola$median, .975),1)}$), 
of MERS-CoV less than 
$\Sexpr{my_round(mean(pstr_gamma_params_mers$median),1)}$ days (95\% CI: 
$\Sexpr{my_round(quantile(pstr_gamma_params_mers$median, .025),1)}$-$\Sexpr{my_round(quantile(pstr_gamma_params_mers$median, .975),1)}$) 
and of smallpox less than 
$\Sexpr{my_round(mean(pstr_gamma_params_smallpox$median),1)}$
days (95\% CI: 
$\Sexpr{my_round(quantile(pstr_gamma_params_smallpox$median, .025),1)}$-$\Sexpr{my_round(quantile(pstr_gamma_params_smallpox$median, .975),1)}$). Additionally, the data suggest that 95\% of cases of Ebola will have an incubation period of less than $\Sexpr{my_round(mean(pstr_gamma_params_ebola$p95),1)}$ days (95\% CI: $\Sexpr{my_round(quantile(pstr_gamma_params_ebola$p95, .025),1)}$-$\Sexpr{my_round(quantile(pstr_gamma_params_ebola$p95, .975),1)}$), of MERS-CoV $\Sexpr{my_round(mean(pstr_gamma_params_mers$p95),1)}$ days (95\% CI: $\Sexpr{my_round(quantile(pstr_gamma_params_mers$p95, .025),1)}$-$\Sexpr{my_round(quantile(pstr_gamma_params_mers$p95, .975),1)}$), and of smallpox $\Sexpr{my_round(mean(pstr_gamma_params_smallpox$p95),1)}$ days (95\% CI: $\Sexpr{my_round(quantile(pstr_gamma_params_smallpox$p95, .025),1)}$-$\Sexpr{my_round(quantile(pstr_gamma_params_smallpox$p95, .975),1)}$).
...
<<figure2-estimated-distribution-dynamic, fig.path='static-figures/', fig.show='hide', fig.height=5, cache=FALSE, eval=TRUE, dev='tiff'>>=
data(kde_smallpox) 
data(kde_ebola)
data(kde_mers)
...
## make plots/data
p_ebola <- plot_risk_uncertainty(pstr_gamma_params_ebola, 
                                 phi=phis, 
                                 nsamp=1000,
                                 u = runif(1000, 1, maxu_ebola),
                                 durations=durs, 
                                 return_plot=TRUE,
                                 return_data=TRUE,
...
<<figure3-scaled-prob-symptoms, fig.path='static-figures/', fig.show='hide', fig.height=3, dev='tiff'>>=
...
<<primary-cost-analysis, results='hide'>>=
...
<<figure4-optimized-cost-analysis, fig.path='static-figures/', fig.show='hide', dev='tiff'>>=
durs <- seq(.5, 5, by=.1)
phis <- c(1/1000, 1/10000)
...
<<optimized-cost-with-uncertainty, cache=TRUE>>=
durs <- seq(.5, 5, by=.1)
phis <- c(1/1000, 1/10000)
...
$\Sexpr{round(min_costs[which(min_costs[,"phi"]==1e-4), "min_cost_dur_days"],1)}$ 
days of monitoring (i.e.,
$\Sexpr{round(min_costs[which(min_costs[,"phi"]==1e-4), "min_cost_dur_days"]/mean(pstr_gamma_params_ebola$median),1)}$ 
median incubation periods, 95\% CI = 
$\Sexpr{round(min_cost_days_distr[which(min_cost_days_distr[,"phi"]==1e-4), "X2.5."],1)}$ 
-
$\Sexpr{round(min_cost_days_distr[which(min_cost_days_distr[,"phi"]==1e-4), "X97.5."],1)}$). 
For the some- or high-risk individuals, the model suggests that the cost is minimized with 
$\Sexpr{round(min_costs[which(min_costs[,"phi"]==1e-3), "min_cost_dur_days"],1)}$ 
days of monitoring 
(i.e.,
$\Sexpr{round(min_costs[which(min_costs[,"phi"]==1e-3), "min_cost_dur_days"]/mean(pstr_gamma_params_ebola$median),1)}$ 
median incubation periods, 95\% CI = 
$\Sexpr{round(min_cost_days_distr[which(min_cost_days_distr[,"phi"]==1e-3), "X2.5."],1)}$ 
-
$\Sexpr{round(min_cost_days_distr[which(min_cost_days_distr[,"phi"]==1e-3), "X97.5."],1)}$) 
...
<<supp-fig2-optimized-cost-analysis, fig.path='static-figures/', fig.show='hide'>>=
### below this, code adapted from chunk ` primary-cost-analysis`
...
## cost matrix construction
## columns represent upper and lower bounds
cost_m <- c(20, 20)     ## per day cost of treatment
cost_trt <- c(4e6, 4e6) ## cost of response to single case
cost_exp <- c(0, 4e6)  ## additional cost of response to single case not captured by monitoring
cost_falsepos <- c(10000, 30000) ## cost of false positive testing

cost_mat2 <- rbind(cost_m, cost_trt, cost_exp, cost_falsepos)
...
### below this, adapted largely from chunk `figure4-optimized-cost-analysis`
durs <- seq(.5, 5, by=.1)
phis <- c(1/1000, 1/10000)
...
ggplot(costs, aes(x=dur*gamma_params['median'], 
    ...
    scale_fill_manual(values=c("#e41a1c", "#377eb8")) +
    scale_color_manual(values=c("#e41a1c", "#377eb8")) +
    geom_segment(aes(x=-100, xend=min_costs2[1, "min_cost_dur_days"], 
                     y=min_costs2[1, "min_cost"], yend=min_costs2[1, "min_cost"]), 
                 linetype=2, color="#377eb8") +
    geom_segment(aes(x=min_costs2[1, "min_cost_dur_days"], xend=min_costs2[1, "min_cost_dur_days"], 
                     y=-1000, yend=min_costs2[1, "min_cost"]), 
                 linetype=2, color="#377eb8") +
    geom_segment(aes(x=-100, xend=min_costs2[2, "min_cost_dur_days"], 
                     y=min_costs2[2, "min_cost"], yend=min_costs2[2, "min_cost"]), 
                 linetype=2, color="#e41a1c") +
    geom_segment(aes(x=min_costs2[2, "min_cost_dur_days"], xend=min_costs2[2, "min_cost_dur_days"], 
                     y=-10000, yend=min_costs2[2, "min_cost"]), 
                 linetype=2, color="#e41a1c") +
    theme(legend.title=element_blank(), legend.position=c(.8, .8))
@
...
$\Sexpr{paste0("\\$", round(min_costs[which(min_costs[,"phi"]==1e-3), "min_cost"],0) - round(min_costs2[which(min_costs2[,"phi"]==1e-3), "min_cost"],0))}$ 
...
\bibliography{active-monitoring}
