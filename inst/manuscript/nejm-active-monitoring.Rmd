...
library(knitr)
theme_set(theme_bw())
data("ebola_gamma_pstr")
data("mers_gamma_posterior")
data("smallpox_gamma_posterior")
```


[NEJM Original Article specs: 2700 words, abstract, a max of 5 tables and figures, up to 40 references.]

# Abstract(250 words)

### Background
Ebola, Middle East Respiratory Syndrome Coronavirus (MERS-CoV), and smallpox are prime examples of pathogens for which active monitoring could play a pivotal role in preventing a large-scale outbreak.

### Methods
We used a probabilistic model to evaluate whether a monitored individual will become symptomatic with the disease of interest during a period of active monitoring. Our model estimates the risks and costs associated with active monitoring programs. Data on the number of individuals actively monitored by New York City during the west African Ebola outbreak and costs associated with the city's overall Ebola response were used as a case-study of active monitoring.

### Results 
During the west African Ebola outbreak, CDC recommended active monitoring for 21 days. This corresponds to about XX times the estimated median incubation period for Ebola of XX days. To tolerate the same level of absolute risk in a future outbreak of either MERS-CoV or Smallpox, our model suggests that a duration of active monitoring could be set at less than 2 times the median incubation period for each pathogen. New York City spent over $1.9 million on their active monitoring program, which monitored 5,321 individuals over XX months. 

### Conclusions
Minimizing unneccessary monitoring could provide valuable savings to public health agencies. Some cost-efficiency may be achieved through simple targeted strategies. As CDC already recommends, it may be appropriate to use exposure risk categories to modify the duration or intensity of active monitoring. Additionally, stronger public health messaging could encourage travelers to take necessary health precautions to avoid common travel-associated illnesses, which would reduce false alarms.
...
The west African Ebola virus disease (Ebola) outbreak was an unprecedented public health emergency that underscored the ease with which pathogens spread in today’s interconnected world. To improve rapid identification and evaluation of individuals at risk for Ebola, on October 27, 2014, the U.S. Centers for Disease Control and Prevention (CDC) recommended active monitoring of individuals in the United States who had potentially been exposed to Ebola virus. Individuals under active monitoring were asked to make daily contact with local health authorities each day for 21 days after their last potential exposure to report their health status. 
...
Public health agencies discontinued active monitoring in February, 2016, when more than 45 days had passed since the last Ebola-affected country was declared free of Ebola virus transmission.[@CentersforDiseaseControlandPrevention:lyxYF_Hp] 
Over 20% of all persons actively monitored for Ebola in the United States were monitored in New York City (NYC), more than any other jurisdiction in the country.[@StehlingAriza:2015vd]

Active monitoring has shown promise as a tool in preventing and responding to outbreaks of pathogens that pose a grave threat to public health. It could play an important role in containing outbreaks of rapidly spreading emerging pathogens, occuring either naturally or via a bioterrorist attack.

The decision to implement active monitoring depends on numerous factors regarding the pathogen of concern and the possible settings of disease transmission. Key considerations include the potential magnitude of an outbreak, the pathogen's transmissibility, the consequences of infection, and the timing of onset of infectiousness in relation to onset of symptoms.[@Fraser:2004ci] Ebola, Middle East Respiratory Syndrome Coronavirus (MERS-CoV), and smallpox are prime examples of known pathogens for which active monitoring could play a pivotal role in preventing a large-scale outbreak.

Active monitoring programs must balance conflicting priorities. The central goals of these programs are to identify, isolate, and treat infected individuals quickly. Setting an active monitoring period substantially longer than any known incubation period of the pathogen of concern could provide greater assurance that an infected individual would exhibit symptoms while being actively monitored. However, to the extent possible, efforts should be made to minimize the costs and detection of false positives through active monitoring. This can be achieved by limiting the monitoring of those with only a minimal risk of the disease of concern and by not monitoring individuals after the risk of the disease onset has passed.

Improving our understanding of the optimal settings for active monitoring is vital for public health preparedness planning. We present a framework for evaluating the risks and costs associated with active monitoring using data from the Ebola response of the NYC Department of Health and Mental Hygiene (DOHMH) and existing data on the incubation periods of Ebola, MERS-CoV, and smallpox. 
This analysis provides empirical evidence that could inform the design and implementation of future active monitoring programs. We illustrate how this framework could be applied to outbreaks of known or emerging pathogens. With the spectre of a global pandemic looming as a constant threat, developing a thorough understanding of how and when active monitoring can add substantial value should be a critical global health priority.


# Methods

### Estimating the incubation period distribution
The incubation period of a disease is the duration of time between exposure to the pathogen and symptom onset.[@Brookmeyer:2005bc] This characteristic is often imperfectly observed in real-world data collection settings.[@Reich:2009jq;@Lessler:2009ub] We obtained previously published incubation period observations on 152 cases of Ebola in Guinea [@Faye:2015jc], 170 laboratory-confirmed cases of MERS-CoV in South Korea[@Virlogeux:2016ty] and 362 cases of smallpox[@Litvinjenko:1972vp;@Mack:1972jc;@Nishiura:2009dh]. We fit parametric distributions to the observed incubation period data using maximum likelihood techniques[@Reich:2009jq] (see Supplement). The data used for these analyses are publicly available[@Virlogeux:dv]. We ran sensitivity analyses to evaluate the influence of several outlying observations in the Ebola dataset.
 
### A model for active monitoring outcomes
We developed a probabilistic model for evaluating the likelihood that a monitored individual would become symptomatic with the disease of interest at different stages of monitoring. Figure 1 presents the model schema, which involves a monitored individual having one of four outcomes: (1) no symptoms warranting clinical followup, (2) no symptomatic infection with the disease of interest, but occurence of symptoms that necessitated ruling out of the disease of interest, (3) symptomatic infection with the disease of interest occurring during the individual's period of active monitoring, (4) symptomatic infection with the disease of interest occurring outside the individual's period of active monitoring. Scenario 4 represents the highest risk of widespread transmission and reduces the chance of a positive clinical outcome for the sick individual. 

Our model calculates the probability of each of the four outcomes, with the associated expected costs, by combining the data on the probabilities of infection, the estimated incubation period distribution, and potential cost ranges for different outcomes. Additionally, it determines the uncertainty associated with these estimates due to not having precise incubation period observations and having an unknown time between exposure and the beginning of monitoring for a particular individual (see Supplement). Our model assumes that infectiousness coincides with the onset of symptoms, a reasonable assumption for these three pathogens.[@Velasquez:2015gv;@Cowling:2015vb;@Fenner:1988ux]
...
```{r}
ebola_gamma_pstr$median <- qgamma(.5, 
                                  shape=ebola_gamma_pstr$shape,
                                  scale=ebola_gamma_pstr$scale)
ebola_gamma_pstr$p95 <- qgamma(.95, 
                                  shape=ebola_gamma_pstr$shape,
                                  scale=ebola_gamma_pstr$scale)
```
...
Estimates and credible regions for incubation period distribution parameters for Ebola, MERS-CoV, and smallpox are shown in Figure 2. Consistent with other studies that used the same data,[@Faye:2015jc;@Virlogeux:2016ty;@Nishiura:2009dh] we estimate that half of all cases of Ebola will have incubation periods of less than `r my_round(mean(ebola_gamma_pstr$median),1)` days (95% CI: `r my_round(quantile(ebola_gamma_pstr$median, .025),1)`--`r my_round(quantile(ebola_gamma_pstr$median, .975),1)`), of MERS-CoV less than `r my_round(mean(mers_gamma_posterior$median),1)` days (95% CI: `r my_round(quantile(mers_gamma_posterior$median, .025),1)`--`r my_round(quantile(mers_gamma_posterior$median, .975),1)`) and of smallpox less than `r my_round(mean(smallpox_gamma_posterior$median),1)` days (95% CI: `r my_round(quantile(smallpox_gamma_posterior$median, .025),1)`--`r my_round(quantile(smallpox_gamma_posterior$median, .975),1)`). Additionally, the data suggest that 95% cases of Ebola will have incubation periods of less than `r my_round(mean(ebola_gamma_pstr$p95),1)` days (95% CI: `r my_round(quantile(ebola_gamma_pstr$p95, .025),1)`--`r my_round(quantile(ebola_gamma_pstr$p95, .975),1)`), of MERS-CoV `r my_round(mean(mers_gamma_posterior$p95),1)` days (95% CI: `r my_round(quantile(mers_gamma_posterior$p95, .025),1)`--`r my_round(quantile(mers_gamma_posterior$p95, .975),1)`), and of smallpox `r my_round(mean(smallpox_gamma_posterior$p95),1)` days (95% CI: `r my_round(quantile(smallpox_gamma_posterior$p95, .025),1)`--`r my_round(quantile(smallpox_gamma_posterior$p95, .975),1)`).
...
```{r figure2-estimated-distribution-dynamic, fig.path='static-figures/', fig.show='hide'}
...
The model showed that an increase in the duration of the active monitoring period is associated with a decline in the probability that an individual develops symptoms due to the disease of concern after the active monitoring period ends. The rate at which that risk declines depends on the estimated variability of the incubation period distribution. Since the estimated incubation period distribution for Ebola showed the highest variability, the probability of symptoms occuring after active monitoring ends decreased more slowly when compared with the other pathogens. 

During the west African Ebola outbreak, CDC recommended active monitoring for 21 days. This corresponds to about `r my_round(21/mean(ebola_gamma_pstr$median),1)` times the estimated median incubation period for Ebola  of `r my_round(mean(ebola_gamma_pstr$median),1)` days (vertical dashed line, Figure 3). To tolerate the same level of absolute risk for either MERS-CoV or Smallpox, our model suggests that a duration of active monitoring could be set at less than 2 times the median incubation period. 
...
tmp <- mean(qgamma(.90, shape=ebola_gamma_pstr$shape, scale=ebola_gamma_pstr$scale))
maxu_ebola <- 2*(tmp - mean(ebola_gamma_pstr$median))

tmp <- mean(qgamma(.90, shape=mers_gamma_posterior$shape, scale=mers_gamma_posterior$scale))
maxu_mers <- 2*(tmp - mean(mers_gamma_posterior$median))

tmp <- mean(qgamma(.90, shape=smallpox_gamma_posterior$shape, scale=smallpox_gamma_posterior$scale))
maxu_smallpox <- 2*(tmp - mean(smallpox_gamma_posterior$median))

## make plots/data
p_ebola <- plot_risk_uncertainty(ebola_gamma_pstr, phi=phis, 
                                 max_u = maxu_ebola,
                                 durations=durs, 
                                 return_plot=TRUE,
                                 return_data=TRUE,
...
p_mers <- plot_risk_uncertainty(mers_gamma_posterior, phi=phis, 
                                max_u = maxu_mers,
...
p_smallpox <- plot_risk_uncertainty(smallpox_gamma_posterior, phi=phis, 
                                    max_u = maxu_smallpox,
                                    durations=durs, return_plot=TRUE,
                                    return_data=TRUE,
...
data_for_scaling <- data_frame(Disease=c("Ebola", "MERS-CoV", "Smallpox"),
                               median_incper=c(mean(ebola_gamma_pstr$median),
                                               mean(mers_gamma_posterior$median), 
                                               mean(smallpox_gamma_posterior$median)))
...
ggplot(all_data, aes(x=d_scaled, y=p50)) + 
    ...
    geom_vline(xintercept=21/mean(ebola_gamma_pstr$median),
...
gamma_params <- c(median = mean(ebola_gamma_pstr$median), 
                shape = mean(ebola_gamma_pstr$shape),
                scale = mean(ebola_gamma_pstr$scale))
durs <- c(1.5, 2, 3, 5)
phis <- c(1/1000, 1/10000)
...
```{r optimized-cost-analysis, fig.cap="Figure 4. Estimated cost ranges for actively monitoring 100 individuals, calculated separately for some or high risk individuals and low (but not zero) risk individuals."}
durs <- seq(.5, 5, by=.1)
phis <- c(1/1000, 1/10000)
...
days of monitoring. For the high-risk individuals, the model suggests that the cost is minimized with around
`r round(min_costs[which(min_costs[,"phi"]==1e-3), "min_cost_dur_days"])` 
days of monitoring (Figure 4). 


# Discussion

For a variety of outbreak-prone pathogens, we quantified the risk, costs, and attendant uncertainty associated with specific durations of active monitoring. This model framework can provide quantiative backing for public health policies in future outbreaks.

Perception of risk is a key component of decision making, but ultimately science should inform public health policies. A key challenge faced by decision makers is that the public's perception of risk is often determined by the consequences even if the probability of the event is small. With severe diseases, the consequences may be perceived as so dire that the public's tolerance for risk will be low, essentially demanding a guaranteed zero probability of an event occurring. While no policy can guarantee zero risk, analyses such as those presented here can assist in determining appropriate thresholds which reduce risk below a tolerable level, even for a risk-averse population.
...
Evaluating the cost of monitoring programs is a challenging, multi-faceted problem. While we present new data from the DOHMH active monitoring program showing that the per-individual cost of monitoring is "low" (about $10–$20 per day), the total program cost can be substantial. Therefore minimizing unneccessary monitoring could provide valuable savings. 

Some cost-efficiency may be achieved through simple targeted strategies. As CDC already recommends, it may be appropriate to use exposure risk categories to modify the duration or intensity of active monitoring. Additionally, stronger public health messaging could encourage travelers to take necessary health precautions to avoid common travel-associated illnesses, e.g. malaria. This in turn could reduce unnecessary testing and costs associated with symptomatic events with diseases other than the one of concern. Low-cost versions of active monitoring have the potential to add value to public health response to less severe outbreaks as well.
