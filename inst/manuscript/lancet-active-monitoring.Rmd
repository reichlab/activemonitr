...
```{r optimized-cost-with-uncertainty, }
durs <- seq(.5, 5, by=.1)
phis <- c(1/1000, 1/10000)
...
min_sampled_costs <- as.data.frame(min_sampled_costs)
min_sampled_costs$phi_lab <- factor(min_sampled_costs$phi, 
                              levels=c(1/1000, 1/10000),
                              labels=c("some or high risk", "low (but not zero) risk"))

costs_distr %>% group_by(phi, phi_lab, dur) %>%
  do(tidy(t(quantile(.$maxcost, probs=c(.05, .5, .95)))))

## minimize explicitly with optim?
