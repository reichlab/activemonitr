...
author: "Nicholas G Reich^1^, Justin Lessler^2^, Jay K Varma^3,4^, Neil M Vora^3,5^"
date: "August, 2016"
output:
  word_document:
    fig_caption: yes
...
^1^Department of Biostatistics and Epidemiology, University of Massachusetts School of Public Health and Health Sciences (Amherst, USA, 01003)  
^2^Department of Epidemiology, Johns Hopkins Bloomberg School of Public Health (Baltimore, USA, 21205)  
^3^Division of Disease Control, New York City Department of Health and Mental Hygiene (New York, USA, 11101)  
^4^Office of the Director, National Center for Emerging and Zoonotic Infectious Diseases, Centers for Disease Control and Prevention (CDC) (Atlanta, USA, 30333)  
^5^Career Epidemiology Field Officer Program, Office of Public Health Practice Preparedness and Response, CDC (Atlanta, USA, 30333)  
...
[\\]:(### http://www.thelancet.com/lancet/information-for-authors/statements-permissions-signatures)


# Abstract

__Background__    
During outbreaks of deadly emerging pathogens (e.g., Ebola, MERS-CoV) and bioterror threats (e.g., smallpox), actively monitoring potentially infected individuals aims to limit disease transmission and morbidity. Guidance issued by CDC on active monitoring was a critical cornerstone of its response to the West Africa Ebola outbreak. There is limited data about how to balance the costs and performance of this important public health activity.


__Methods__    
We estimate the risks and costs associated with specific durations of active monitoring for three pathogens of significant public health concern: MERS-CoV, Ebola, and smallpox. Specifically, we model the risk of individuals becoming symptomatic from the disease of interest and other causes. As a case-study, we analyze New York City's Ebola active monitoring program, using data on the costs of active monitoring, treatment of an Ebola case, and the number of monitored individuals over a 16-month period.

__Findings__    
For monitored individuals in the CDC exposure-risk categories of "low (but not zero) risk" or "some or high risk" of Ebola, we estimated the probabilities of infection to be 1/10,000 and 1/1,000, respectively. Using these estimates, our model identified unique durations of active monitoring for Ebola that would minimize expected costs for each scenario: 21 and 31 days, respectively. Extending our analysis to smallpox and MERS-CoV, we found that the optimal length of active monitoring relative to the median incubation period was reduced compared to Ebola due to less variable incubation periods. 

__Interpretation__    
Active monitoring can save lives and minimize resource-intensive response to high-consequence events, but the costs of monitoring and responding to false alarms are also significant. These costs can be minimized by using simple targeted strategies that are evidence-based. Resources can be most effectively allocated by using exposure-risk categories to modify the duration or intensity of active monitoring. 

__Funding__    
The authors received no funding for the design or writing of this manuscript.

# Research in context

__Evidence before this study__    
We searched the ISI Web of Science using the keywords "active monitoring" and "outbreak". Several recent studies were identified that summarize and describe active monitoring programs that were in place during the 2014-2015 West Africa Ebola outbreak. No existing studies provide information or data on the cost of these programs or models to project or estimate costs for future implementations of active monitoring.

__Added value of this study__    
Active monitoring is a relatively new countermeasure used by public health officials to combat emerging infectious disease outbreaks. However, there are limited data about how to balance the cost and performance of this important public health activity. Our study is the first to explicitly assess the risks and costs associated with active monitoring programs, providing a flexible model that can be used for a wide range of outbreak scenarios. The model presented in this work integrates data from New York City, some of which are previously unpublished, on the costs of active monitoring, treatment of an Ebola case, and the number of monitored individuals over a 16-month period.

__Implications of all the available evidence__    
During outbreaks, active monitoring programs can be used to monitor potentially exposed individuals for symptoms. 
While the financial and social costs can be substantial, such programs can be used to improve clinical outcomes for monitored individuals and reduce the risk of subsequent transmissions.
...
For diseases with asymptomatic transmission –such as influenza or measles– detection of new cases based on active monitoring of symptomology might not be soon enough to prevent secondary transmission. 
...
We developed a model that uses varying probabilities to alter the incubation period, thus estimating whether active monitoring would identify an individual infected with the pathogen of interest. Figure 1 presents the model schema, which involves a monitored individual having one of four outcomes: 
...
![Figure 1. Model schematic representing four outcome scenarios for a person under active monitoring. Estimated costs shown are based on published costs[@Yacisin:2015wm] as well as new data from DOHMH and Bellevue Hospital in NYC. Details on the model formulation are available in Supplemental Table 3. Our model uses probabilities to calculate the likelihood of each of the possible model outcomes. Additionally, we estimate the probability that an individual who does not develop the disease of interest develops symptomatic illness necessitating hospitalization to rule-out the disease of interest. (See Supplement for details.)](static-figures/figure1-model-schema-v9.jpg)

Using this model, we calculate the probability of each of the four outcomes, with the associated expected costs, by combining the data on the probabilities of infection, the estimated incubation period distribution, and potential cost ranges for different outcomes. Additionally, it determines the uncertainty associated with these estimates due to not having precise incubation period observations and having an unknown time between exposure and the beginning of monitoring for a particular individual (see Supplement). For these pathogens, we assume that infectiousness coincides with the onset of symptoms.[@Velasquez:2015gv;@Cowling:2015vb;@Fenner:1988ux]
...
We developed open-source software, including a freely-available web application at [http://iddynamics.jhsph.edu/apps/shiny/activemonitr/](http://iddynamics.jhsph.edu/apps/shiny/activemonitr/). All analyses were run in `r R.version.string`.[@rcoreteam] These tools enable others to easily implement our model.
...
```{r fig-3, fig.cap="Figure 3. Estimated probabilities of symptoms occurring after active monitoring (AM) ends across different active monitoring period durations, shown as multiples of the median incubation period. Figures are shown for 'some or high risk' and 'low (but not zero) risk' scenarios, with probabilities of developing symptomatic infection set to 1/1,000 and 1/10,000, respectively. The vertical dashed line indicates the 21 day duration recommended for Ebola active monitoring (i.e., approximately 2.4 times the median incubation period of Ebola).", fig.height=3}
...
```{r optimized-cost-with-uncertainty, cache=TRUE}
durs <- seq(.5, 5, by=.1)
phis <- c(1/1000, 1/10000)
...
Our model provides ranges of expected cost for active monitoring systems. It identifies an optimal duration of active monitoring, by finding the expected cost range with the lowest maximum value. We applied the model to the case-study of Ebola in NYC, based on data from DOHMH (Table 1, Figure 4). These figures reflect the fact that as the active monitoring period duration increases, the probability of the highest-cost events (an individual developing the disease of interest after active monitoring ends) decreases. However, at the same time, the cost of active monitoring and possible testing due to non-specific symptoms increases as more person-days are included in the active monitoring period. For the low-risk individuals, the model suggests that the cost is minimized with  
...
For the high-risk individuals, the model suggests that the cost is minimized with 
...
![Figure 4. Estimated cost ranges of actively monitoring 100 individuals for Ebola, calculated separately for some or high risk individuals and low (but not zero) risk individuals. The dashed lines intersect at the minimum point for the upper limit of each cost range.](static-figures/optimized-cost-analysis-1.png)
...
However, for diseases with asymptomatic transmission –such as influenza or measles– detection of the cases based on active monitoring of symptomology might not be soon enough to prevent secondary transmission.  As such, for these diseases, quarantine might be a more appropriate public health measure.
...
While our model explicitly propagates uncertainty in the incubation period distribution, a formal, data-driven understanding of the uncertainty for other model parameters is not likely to be available to public health practitioners in practice due to lack of reliable data. Therefore, we ran the model for plausible ranges of other parameters in order to present a range of possible costs and risks. Sensitivity to these assumptions can be assessed through the open-access web applet that implements the model.
...
Recent history has shown that the unexpected emergence of new disease threats has become a recurring theme in global health and preparedness. While active monitoring may not play a role in the response to every emerging disease threat (e.g., it has not played an important role in the response to the Zika virus epidemic due to the high percentage of asymptomatic cases), it will be an important tool in the response to most. Our framework provides valuable information for assessing the cost-effectiveness of various active monitoring strategies in response to critical disease outbreaks. By providing an empirical basis for evaluating active monitoring programs, these tools can strengthen biosecurity and optimize active monitoring programs in response to future global disease threats.

# Disclosures
The findings and conclusions in this article are those of the authors and do not necessarily represent the official position of DOHMH or CDC. The authors report no conflicts of interest.
...
# Acknowledgments
The authors received no funding for the design or writing of this manuscript. We thank the individuals who underwent active monitoring in NYC, Ifeoma Ezeoke for preparing the DOHMH active monitoring data and Margaret Pletnikoff for preparing the DOHMH cost data. We thank Dr. Simon Cauchemez, PhD (Institut Pasteur, Paris, France), Dr. Amadou Sall, PhD (Institut Pasteur, Dakar, Senegal) and Dr. Ousmane Faye, PhD (Institut Pasteur, Dakar, Senegal) for providing access to the Ebola incubation period data from Guinea and giving permission for this data to be made publicly available. We also acknowledge John Maher, Esq., for providing details on costs of evaluating Ebola patients at Bellvue Hospital in New York City.
